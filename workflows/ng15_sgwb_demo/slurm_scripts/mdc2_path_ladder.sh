#!/bin/bash
# M1 task 1.7 (estimator B): the path-sampling ladder on MDC2 Stage C.
#
...
#       --from-runs 'outputs/mdc2_ladder_eps*/mdc2_ladder_eps*_results.nc' \
#       --evaluate configs/mdc2_ladder_eps000.ini \
#       --out outputs/lnb_path_sampling.json
...
#SBATCH --output=/fred/oz022/tkimpson/Argus/workflows/ng15_sgwb_demo/outputs/logfiles/mdc2_ladder_%A_%a.out
...
CONFIG="${ROOT}/configs/mdc2_ladder_eps${TAG}.ini"
sed -e "s/__EPS_VALUE__/${EPS}/" -e "s/__EPS_TAG__/${TAG}/" \
    "${ROOT}/configs/mdc2_ladder_rung.ini.template" > "${CONFIG}"
